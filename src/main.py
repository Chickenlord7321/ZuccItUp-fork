...
class LogoutException(Exception):
    ...
    # Allows you to print the exception, e.g.:
...
def check_logout_or_quit(answer: str) -> str:
    if answer.lower() == "quit":
        ...
        raise LogoutException("User typed: " + answer)
    return answer
...
def input_str(msg: str, regex_pattern: str = None) -> str:
    while True:
        ...
        try:
            ...
            if regex_pattern is None:   # We don't check for regex pattern-matching
                return answer
            elif re.match(regex_pattern, answer):   # We check that the user's input matches the right pattern
                return answer
            else:                       # User input did not match the required pattern
                print("Sorry, that was not a valid answer. Please try again.")
        except LogoutException:         # User logged out
            raise LogoutException("User typed: " + answer)
...
def input_int(msg: str, minimum: int = -sys.maxsize - 1, maximum: int = sys.maxsize) -> int:
    while True:
        try:
            answer = check_logout_or_quit(input(msg))
            value = int(answer)
            if minimum <= value <= maximum:     # value is between min and max (inclusive)
                return value
            print(f"Please enter a number between {minimum} and {maximum}.")    # Value is too big or too small
        except ValueError:                      # Value was not a number
            print("Please enter a valid number.")
            continue                            # User logged out
        except LogoutException:
            raise LogoutException("User logged out")
...
def login_or_signup():
    while True:
        try:
            ...
            print("Sorry, you are not logged in yet. Please select an option.")
            continue
...
def _browse_and_add(menu_obj: Menu, cart: Cart):
    ...
    try:
        ...
        return LogoutException("user logged out")
# end _browser_and_add()
...
def run_customer():
    ...
    while True:
        ...
        try:
            ...
            raise LogoutException("Customer logged out")
...
def _confirm_received_flow(customer: Customer):
    ...
    order.update_status(Status[doc.get("orderStatus", "")])
...
def run_agent():
    ...
    while True:
        ...
        try:
            ...
            raise LogoutException("Agent logged out")
...
def _view_available_deliveries(agent: DeliveryAgent):
    ...
    while True:
        ...
        try:
            ...
            order.update_status(Status[selected.get("orderStatus", "")])
...
def _manage_order_in_progress(agent: DeliveryAgent, order_doc: dict):
    ...
    while True:
        ...
        order.update_status(Status[current_status])
